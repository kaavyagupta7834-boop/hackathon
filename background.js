...
async function evaluateRepo(tabId, owner, repo) {
  try {
    ...
    // Step 2b: does the repo actually contain any code yet?
    // We treat the repo as "empty" if any of these are true:
    //   - the GitHub "size" field is 0 (no content at all), OR
    //   - pushed_at is missing (no push has ever happened), OR
    //   - pushed_at equals created_at (the repo was created but never pushed to —
    //     GitHub initializes pushed_at to the creation time in that case).
    // In any of these cases we stop here: no badge, no button, just a message.
    const hasNoCommits = !pushedAt || pushedAt === createdAt;
    const isEmpty = size === 0 || hasNoCommits;
